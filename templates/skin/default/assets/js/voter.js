...
ls.voter = (function ($) {
    ...
    this.closeDiv = function () {
                ...
                ls.ajax(url, params_to, function (response) {
                    ...
                    if (response.bStateError) {
                        ...
                        if (response.rating > 0) {
                            plus_minus = '+';
                            rating_class = ' class="green" ';
                        }
                        if (response.rating < 0) {
                            rating_class = ' class="red" ';
                        }
        ...
        this.getInfoQuestion = function (params) {
            ls.voter.what = 'question';
            if ($('#vs-vote').length) {
                $('#vs-vote .vs_plus_users').height(0);
                ...
                ls.ajax(url, params_to, function (response) {
                    ...
                    if (response.bStateError) {
                        ...
                        $('#vs-vote .vs_header').html(response.vote_for_this + ' - <em>' + response.count_votes + '</em>');
                        ...
                        if (((plusCol - 1) / 10) < 1) {
                            ...
                            html_addplus2 += ls.voter.createUsersLi(plusArray[1]);
                            html_addplus2 += '</ul>';
                        }

                        $('#vs-vote .vs_plus_users').html(html_addplus);
                        $('#vs-vote .vs_minus_users').html(html_addplus2);
                        $('#vs-vote').css({
                            'display': 'block',
                            'position': 'absolute',
                            'top': '-10000px',
                        ...
                        if (putToBottom) {
                            ...
                            $('#vs-vote .vs_arrow_bubble_top').css('display', 'block');
                            $('#vs-vote .vs_arrow_bubble_top').css('background-position', '0px 0');
                            $('#vs-vote .vs_arrow_bubble_bottom').css('display', 'none');
                        } else {
                            $('#vs-vote').css({
                            ...
                            $('#vs-vote .vs_arrow_bubble_top').css('display', 'none');
                            $('#vs-vote .vs_arrow_bubble_bottom').css('display', 'block');
                            $('#vs-vote .vs_arrow_bubble_bottom').css('background-position', '0px 0');
                        }
                        ls.voter.opened = true;
                        ls.voter.height_td = $('#vs-vote .vs_plus_users').height();
                    }
        ...
        this.prevPage = function () {
            if (ls.voter.what == 'question') {
                koef = 2;
            } else {
                koef = 1;
            }
            ls.voter.setPage(ls.voter.activePage - 1 * koef);
        },
        ...
        this.setPage = function (num) {
            ...
            if (num != ls.voter.activePage) {
                ...
                var html_addminus = '';
                if (ls.voter.what == 'question') {
                    $('#vs-vote .vs_paginator a:eq(' + num / 2 + ')').addClass('active');
                    if (ls.voter.plusArray[num + 1]) {
                        html_addminus += ls.voter.createUsersLi(ls.voter.plusArray[num + 1]);
                    }
                } else {
